...
import { useGetInventorySubGroups } from '../../hooks/inventoryHooks/useGetInventorySubGroup';
import { 
  Package, 
  Search, 
  AlertCircle, 
...
const InventorySubGroupsTable = () => {
  const { data: inventoryGroups, isLoading, error } = useGetInventorySubGroups();
  ...
  // Extract groups from response - handle array directly
  const groups = useMemo(() => {
    if (!inventoryGroups) return [];
    // If it's already an array, use it directly
    if (Array.isArray(inventoryGroups)) return inventoryGroups;
    // If it's wrapped in an object, try to extract the array
    if (inventoryGroups.data && Array.isArray(inventoryGroups.data)) return inventoryGroups.data;
    return [];
  }, [inventoryGroups]);
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        ...
        <div className="max-w-full mx-auto">
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Error Loading Inventory SubGroups</h3>
            <p className="text-gray-600 mb-2">Failed to fetch inventory subgroups</p>
            {error.message && (
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Inventory SubGroups</h1>
              <p className="text-gray-600 mt-1">Manage and view all inventory subgroup categories</p>
            </div>
            <button
        ...
        <div className="mb-6 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="text-sm text-gray-600 font-medium">
              Showing {filteredGroups.length > 0 ? startIndex + 1 : 0} to {Math.min(endIndex, filteredGroups.length)} of {filteredGroups.length} inventory subgroups
              {groups.length > 0 && filteredGroups.length !== groups.length && (
        ...
        {filteredGroups.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No Inventory SubGroups Found</h3>
            <p className="text-gray-600">
              {searchTerm ? 'Try adjusting your search terms.' : 'No inventory subgroups are available at the moment.'}
            </p>
          </div>
        ) : (
          <>
...
export default InventorySubGroupsTable;